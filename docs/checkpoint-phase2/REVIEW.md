...
| 파일 | 오류 | 우선순위 |
|------|------|----------|
| `Store.ts:6` | BattleState import 충돌 | Phase 3 |
...
| `game.ts:472` | toggleDesigner 타입 비교 | Phase 4 |
...
| 모듈 | 테스트 수 | 커버 항목 |
|------|----------|-----------|
| GameManager.test.ts | 15 | eraseComponent, saveSpell(3), loadSpell, renderDesigner(2), clearDesign, recordRun(4), trimComponents(2) |
...
### 누락된 테스트
- ❌ `startLoop()` / game loop tick
- ❌ `updateBattleTick()` with survival speed
- ❌ `bossSpawned` flag
- ❌ `checkUnlocks()` with mana bonus
- ❌ `clearAllData()`
- ❌ `getTotalStars(includeCurrentRun=false)`

---

## 6. 권장 사항

1. Phase 3 전에 `clearAllData` 단위 테스트 추가
2. `Storage.ts`의 import를 파일 상단으로 이동
3. Phase 3에서 Store.ts BattleState 충돌 해결
